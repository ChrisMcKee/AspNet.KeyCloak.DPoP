...
public class TokenValidationHandler : DPoPEventHandlerBase, IDPoPEventHandler<TokenValidatedContext>
{
    ...
    internal virtual async Task HandleAllowedMode(TokenValidatedContext context)
    {
        ...
        if (!IsDPoPScheme(context.Request))
        {
            if (IsDPoPProofHeaderExists(context.Request) || IsTokenDPoPBound(context))
            {
                ...
                context.HttpContext.Items[KeyCloakConstants.DPoP.BearerErrorDescription] =
                    KeyCloakConstants.DPoP.Error.Description.BearerSchemeWithDPoPProof;
                context.HttpContext.Items[KeyCloakConstants.DPoP.BearerStatusCode] = HttpStatusCode.Unauthorized;
                context.Fail(KeyCloakConstants.DPoP.Error.Description.BearerSchemeWithDPoPProof);
                return;
            }
    ...
    internal DPoPProofValidationParameters CreateDPoPProofValidationParameters(TokenValidatedContext context)
    {
        ...
        {
            throw new InvalidOperationException("KeyCloak DPoP options not configured.");
        }
    ...
    internal string GetDPoPProofToken(TokenValidatedContext context)
    {
        KeyValuePair<string, StringValues> header = context.Request.Headers
            .FirstOrDefault(h =>
                string.Equals(h.Key, KeyCloakConstants.DPoP.ProofHeader, StringComparison.OrdinalIgnoreCase));
        ...
        Action<TokenValidatedContext> handleInvalidRequest)
    {
        ...
        {
            case KeyCloakConstants.DPoP.Error.Code.InvalidToken:
                SetErrorContext(context, validationResult, HttpStatusCode.Unauthorized);
                break;

            case KeyCloakConstants.DPoP.Error.Code.InvalidRequest:
                handleInvalidRequest(context);
                break;

            case KeyCloakConstants.DPoP.Error.Code.InvalidDPoPProof:
        ...
        HttpStatusCode statusCode)
    {
        ...
        context.HttpContext.Items[KeyCloakConstants.DPoP.DPoPErrorCode] = validationResult.Error;
        context.HttpContext.Items[KeyCloakConstants.DPoP.DPoPStatusCode] = statusCode;
        context.HttpContext.Items[KeyCloakConstants.DPoP.DPoPErrorDescription] = validationResult.ErrorDescription;
        context.Fail((validationResult.ErrorDescription ?? validationResult.Error) ??
                     KeyCloakConstants.DPoP.Error.Description.UnknownError);
    }

    private bool IsTokenDPoPBound(TokenValidatedContext context)
    {
        return context.Principal?.HasClaim(c => c.Type == KeyCloakConstants.DPoP.Cnf) ?? false;
    }
