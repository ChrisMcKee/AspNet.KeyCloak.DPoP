...
///     according to the KeyCloak and OAuth DPoP specifications.
/// </summary>
public class DPoPProofValidationService : IDPoPProofValidationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            result.SetError(KeyCloakConstants.DPoP.Error.Code.InvalidRequest,
                KeyCloakConstants.DPoP.Error.Description.DPoPProofMissing);
            return result;
        }
        ...
        {
            result.SetError(KeyCloakConstants.DPoP.Error.Code.InvalidRequest,
                KeyCloakConstants.DPoP.Error.Description.AccessTokenMissing);
            return result;
        }
    ...
    {
        Claim? cnfClaim =
            validationParameters.AccessTokenClaims?.FirstOrDefault(c => c.Type == KeyCloakConstants.DPoP.Cnf);

        if (cnfClaim?.Value is null or "")
        {
            validationResult.SetError(
                KeyCloakConstants.DPoP.Error.Code.InvalidToken, KeyCloakConstants.DPoP.Error.Description.CnfClaimMissing);
            return;
        }
        ...
        if (!TryParseCnfClaim(cnfClaim.Value, out Dictionary<string, JsonElement>? cnfJson))
        {
            validationResult.SetError(
                ...
                KeyCloakConstants.DPoP.Error.Description.InvalidCnfClaim);
            return;
        }
        ...
        {
            validationResult.SetError(
                KeyCloakConstants.DPoP.Error.Code.InvalidToken, KeyCloakConstants.DPoP.Error.Description.InvalidSignature);
        }
    }
    ...
    internal bool TryExtractJsonWebKey(JsonWebToken? token, out string jwkJson)
    {
        ...
        if (token == null || !token.TryGetHeaderValue(KeyCloakConstants.DPoP.JsonWebKey, out jwkValues))
        {
            return false;
        }
    ...
    {
        if (!cnfJson.TryGetValue(KeyCloakConstants.DPoP.JwkThumbprint, out JsonElement jktJson))
        {
            return false;
        }
    ...
    internal bool ValidateAccessTokenHash(string? accessToken, DPoPProofValidationResult validationResult)
    {
        ...
        if (validationResult.ProofClaims != null &&
            !validationResult.ProofClaims.TryGetValue(KeyCloakConstants.DPoP.Ath, out ath))
        {
            return false;
        }
    ...
    {
        if (!proofClaims.TryGetValue(KeyCloakConstants.DPoP.Jti, out var jti))
        {
            return false;
        }
    ...
    {
        return proofClaims.TryGetValue(KeyCloakConstants.DPoP.Htm, out var htm) &&
    ...
    {
        return proofClaims.TryGetValue(KeyCloakConstants.DPoP.Htu, out var htu) &&
        ...
        DPoPProofValidationResult validationResult)
    {
        if (validationResult.ProofClaims is null ||
            !validationResult.ProofClaims.TryGetValue(KeyCloakConstants.DPoP.Iat, out var iat))
        {
            return false;
        }
